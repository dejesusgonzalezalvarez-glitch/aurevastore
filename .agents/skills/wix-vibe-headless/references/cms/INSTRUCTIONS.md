...
```js
const { accessToken } = await db.asServiceRole.connectors.getConnection("wix");  // admin-level
...
asset on Wix: `db.integrations.Core.UploadFile` parks the file on Base44 instead, which splits the
source of truth away from the site.

**Authorize that function against the Wix member**, using the member token the client sends.
`db.auth.me()` resolves the *Base44* account — you, the builder — so a function gated on it turns
