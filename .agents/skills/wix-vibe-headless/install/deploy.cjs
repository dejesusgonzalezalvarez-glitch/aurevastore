...
// Post-install deploy — run by db.md STEP 1 with the vertical(s) the app needs:
...
// (imports base44Client / calls db.auth.*). Never touch a file that's already the Wix version
...
function isBase44AuthBoilerplate(path) {
  ...
  return src.includes('@/api/base44Client') || src.includes('db.auth.');
}
